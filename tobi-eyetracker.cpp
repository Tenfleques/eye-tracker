...
void getGazeFrame(tobii_gaze_point_t const* gaze_point){
    static VideoCapture cap(0);
    static long index = 0;
    Mat frame;
    if(!cap.isOpened()){
        cap >> frame; // get a new frame from camera
        // save the frame, do any other processing we might need to do 
    }

    // we have the index, the gaze positions and the frame 
    // index, gaze_point->position_xy[ 0 ], gaze_point->position_xy[ 1 ], frame
}
