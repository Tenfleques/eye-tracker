...
class Root(FloatLayout):
    ...
    def eyetracker_ready(self):
        recent_gazes = list(tobii_listener.getTopRecords())
        
        self.ready_device= len(recent_gazes)
        ...
        if not self.ready_device:
            ...
            current = time.time()
            last_reading = recent_gazes[0].sys_clock
    ...
    def playVideo(self):
        if "capture" in props(self):
            ...
            if self.capture.isOpened() and fps:
                ...
                Thread(target=self.loopRecord, args =(lambda : not self.video_play, )).start()
                # Clock.schedule_interval(self.video_update, 0.001)
                self.applog("")
            else:
                self.applog(self.get_local_str("_video_error"))
    
    def loopRecord(self, stop):
        print(stop())
        while not stop():
            self.video_update(time.time())
            time.sleep(0.05)
        pass
    ...
    def video_update(self, dt):
        ...
        if not ret:
            ...
            if not dt == None:
                # save the frame_id along with the gaze data
